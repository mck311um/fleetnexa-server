...
const getProducts = async () => {
    try {
        const response = await dodo_config_1.default.products.list();
        ...
        await prisma_config_1.default.$transaction(async (tx) => {
            for (const product of products) {
                const productData = {
                    ...
                    priceXCD: parseFloat((((product.price || 0) * 2.712) / 100).toFixed(2)),
                };
...
const getTransactions = async () => {
    try {
        const response = await dodo_config_1.default.payments.list();
        ...
        await prisma_config_1.default.$transaction(async (tx) => {
            for (const payment of payments) {
                ...
                const subscription = await dodo_config_1.default.subscriptions.retrieve(subscriptionId);
                const paymentData = {
