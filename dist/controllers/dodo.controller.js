...
const dodo_config_1 = __importDefault(require("../config/dodo.config"));
...
const webhook = new standardwebhooks_1.Webhook(WEBHOOK_KEY);
const createDodoPayment = async (req, res, next) => {
    const { body } = req.body;
    try {
        const dodoResponse = await dodo_config_1.default.subscriptions.create({
            billing: {
                city: body.city,
...
const getInvoice = async (req, res, next) => {
    ...
    try {
        const payment = await dodo_config_1.default.payments.retrieve(paymentId);
        if (!payment) {
            return res.status(404).json({ error: 'Payment not found' });
        }
        const invoice = await dodo_config_1.default.invoices.payments.retrieve(payment.payment_id);
        if (!invoice) {
...
const dodoPaymentWebhook = async (req, res, next) => {
    try {
        const body = req.body;
...
const handleSubscriptionActive = async (data) => {
    try {
        ...
        await prisma_config_1.default.tenantSubscription.upsert({
